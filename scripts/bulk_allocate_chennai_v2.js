...
async function main() {
    console.log("Starting allocation v2...");

    // 1. Find Chennai Center
    ...
    if (!centers || centers.length === 0) {
        console.error("No Chennai center found. Aborting.");
        return;
    }

    const chennai = centers[0];
    console.log(`Using Center: ${chennai.center_name} (${chennai.center_id})`);

    // 2. Find or Create Trial
    let { data: trials } = await supabase
        .from('trials')
        .select('*')
        .eq('center_id', chennai.center_id);

    let trialData;
    if (trials && trials.length > 0) {
        trialData = trials[0];
        console.log(`Using Existing Trial: ${trialData.trial_name}`);
    } else {
        console.log("Creating new trial...");
        const { data: newTrial, error: createError } = await supabase
            .from('trials')
            .insert({
                center_id: chennai.center_id,
                trial_name: 'Chennai Selection Trial',
                trial_venue: chennai.center_name,
        ...
        if (createError) {
            console.error("Failed to create trial:", createError.message);
            return;
        }
        trialData = newTrial;
        console.log(`Created New Trial: ${trialData.trial_name}`);
    }

    // 3. Match and Allocate
    ...
    const { data: completedRegs, error: regError } = await supabase
        .from('player_workflow')
        .select('*');

    if (regError) {
        console.error("Error fetching regs:", regError.message);
        return;
    }

    const matches = [];
    targets.forEach(t => {
        const found = completedRegs.find(p =>
            (p.email && p.email.toLowerCase() === t.email) ||
            (p.phone && p.phone.replace(/[^0-9]/g, '') === t.phone)
        );
    ...
    // Allocate - WITHOUT TRIAL_ID
    const allocations = matches.map(p => ({
        workflow_id: p.workflow_id,
        // trial_id: trialData.trial_id, // REMOVED
        allocation_date: trialData.trial_date,
        allocation_venue: trialData.trial_venue,
        allocation_time: trialData.trial_time,
        allocation_batch: trialData.trial_batch,
        attendance_status: 'pending',
        selection_status: 'pending' // Shows as Allocated
    }));

    const { data: existingAlloc } = await supabase
        .from('trials_allocations')
        .select('workflow_id');
    const existingIds = new Set(existingAlloc ? existingAlloc.map(a => a.workflow_id) : []);

    const toInsert = allocations.filter(a => !existingIds.has(a.workflow_id));
    console.log(`New allocations to insert: ${toInsert.length}`);
    ...
    if (toInsert.length > 0) {
        ...
        if (insertError) {
            console.error("Insert Error:", insertError.message, JSON.stringify(insertError, null, 2));
        } else {
            console.log("Allocations inserted successfully.");

            // Update workflow stage
            const workflowIds = toInsert.map(a => a.workflow_id);
            const { error: updateError } = await supabase
                .from('player_workflow')
                .update({ workflow_stage: 'trials_allocated' })
                .in('workflow_id', workflowIds);

            if (updateError) console.error("Update Stage Error:", updateError.message);
            else console.log("Workflow stages updated.");
        }
    } else {
        console.log("No new allocations needed.");
    }
}
