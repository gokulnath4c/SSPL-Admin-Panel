...
async function importUnmatched() {
    console.log("Identifying unmatched players...");

    // 1. Process target list
    ...
    // 2. Scan DB for existing (to avoid duplicates)
    // Fetch all phones/emails for lookup
    ...
    const knownEmails = new Set(allRegs.map(r => r.email?.toLowerCase()));
    const knownPhones = new Set(allRegs.map(r => r.phone?.replace(/[^0-9]/g, '')));

    // 3. Filter New Players
    const newPlayers = targets.filter(t => {
        const emailExists = knownEmails.has(t.email);
        const phoneExists = knownPhones.has(t.phone);
        return !emailExists && !phoneExists;
    });

    console.log(`Total Targets: ${targets.length}`);
    console.log(`Already Existing: ${targets.length - newPlayers.length}`);
    ...
    const regPayload = newPlayers.map(p => {
        // Generate Name from Email
        const namePart = p.email.split('@')[0];
        // Replace . _ with space and Title Case
        const fullName = namePart
            .replace(/[._]/g, ' ')
            .replace(/\d+/g, '') // Remove numbers from name? Optional. Let's keep it simple.
            .split(' ')
        ...
        return {
            ...
            pincode: '600000',  // Default
            date_of_birth: '2000-01-01', // Fixed column name
            position: 'Batsman', // Replaces role
            // gender: 'Male',     // Unknown col name?
            // role: 'Batsman',    // Unknown col name?
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString()
        };
    });

    console.log("Inserting Registrations (iterative fix)...");
    const { data: insertedRegs, error: insRegError } = await supabase
        .from('player_registrations')
        .insert(regPayload)
        .select('id, email, phone, full_name'); // Need ID for Workflow

    if (insRegError) {
        console.error('Reg Insert Error:', JSON.stringify(insRegError, null, 2));
        fs.writeFileSync('last_error.txt', JSON.stringify(insRegError, null, 2));
        return;
    }
    console.log(`Inserted ${insertedRegs.length} registrations.`);


    // 5. Create Workflows (skip straight to trials_allocated)
    const workflowPayload = insertedRegs.map(r => ({
        registration_id: r.id,
        workflow_stage: 'trials_allocated', // SKIP TRIALS LIST
        payment_status: 'captured',
    ...
    const { data: insertedWorkflows, error: insWfError } = await supabase
        .from('player_workflow')
        .insert(workflowPayload)
        .select('workflow_id');

    if (insWfError) { console.error('Workflow Insert Error:', JSON.stringify(insWfError, null, 2)); return; }
    console.log(`Inserted ${insertedWorkflows.length} workflows.`);

    // 6. Allocate to Trial
    // Get Trial ID (Chennai Bulk Allocation)
    const { data: trial } = await supabase
        .from('trials')
        .select('trial_id')
        .eq('trial_name', 'Chennai Bulk Allocation')
        .single();

    if (!trial) { console.error('Trial "Chennai Bulk Allocation" not found! Run migration script first?'); return; }

    const allocPayload = insertedWorkflows.map(w => ({
    ...
    const { error: insAllocError } = await supabase
        .from('trial_allocations')
        .insert(allocPayload);

    if (insAllocError) console.error('Allocation Insert Error:', JSON.stringify(insAllocError, null, 2));
    else console.log("Success! All unmatched players imported and allocated.");
}
