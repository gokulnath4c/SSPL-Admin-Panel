...
async function audit() {
    ...
    // 2. Fetch all completed registrations (for matching)
    const { data: regs, error } = await supabase
        .from('player_workflow')
        .select('workflow_id, email, phone, workflow_stage');

    if (error) { console.error(error); return; }

    console.log(`DB Registrations: ${regs.length}`);
    ...
    for (const t of targets) {
        const found = regs.find(r => {
            ...
            return (r.email && r.email.toLowerCase() === t.email) ||
                (rPhone && rPhone.includes(t.phone)); // Try includes for phone if lengths differ
        });
        ...
        if (found) {
            ...
            notMatched++;
            // console.log(`No match for: ${t.email} | ${t.phone}`);
            // Log first 5 misses
            if (notMatched <= 5) console.log(`Missed: ${t.email} | ${t.phone}`);
        }
    }

    console.log(`Total Matched: ${matched}`);
    console.log(`Total Missed: ${notMatched}`);
}
