...
interface Player {
    ...
    level_2_status?: 'pending' | 'selected' | 'not_selected' | 'absent';
...
const TrialLevel2Tab = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            setIsLoading(true);
            setLoadingMessage('Loading Level 2 players from Database...');

            try {
                // Fetch players who passed Level 1 directly from DB
                const res = await getTrialLevelPlayers({ level_1_status: 'selected' });
                ...
                if (res.success && res.data) {
                    const mapped: Player[] = res.data.map(p => ({
                        ...
                        attendance_status: p.level_2_status === 'absent' ? 'absent' : (p.level_2_status && p.level_2_status !== 'pending' ? 'attended' : 'pending'),
                        level_2_status: p.level_2_status || 'pending',
                        level_2_score: p.level_2_score || 0,
                        remarks: p.level_2_remarks || ''
                    }));
    ...
    const PlayerRow = ({ player }: { player: Player }) => {
        ...
        const [formData, setFormData] = useState({
            score: player.level_2_score || '',
            remarks: player.remarks || ''
        });
        const [selectionStatus, setSelectionStatus] = useState<'pending' | 'selected' | 'not_selected' | 'absent'>(player.level_2_status || 'pending');
        ...
        const onSave = async () => {
            ...
            const res = await updateTrialLevelProgress(player.workflow_id, {
                level_2_status: selectionStatus,
                level_2_score: Number(formData.score) || 0,
                level_2_remarks: formData.remarks
            });
        ...
        <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-blue-800 text-sm">
                    <strong>Trial Level 2:</strong> Players marked as "Selected for Level 2" in Level 1. Mark attendance, enter scores for present players, and set final selection status.
                </p>
            </div>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs text-gray-500 uppercase bg-gray-50 border-b">
                            <tr>
                                ...
                                <th className="px-4 py-3 font-medium">L1 Status</th>
                                <th className="px-4 py-3 font-medium">Attendance</th>
                                <th className="px-4 py-3 font-medium">Score</th>
                                <th className="px-4 py-3 font-medium">Remarks</th>
                                <th className="px-4 py-3 font-medium">L2 Selection</th>
                            </tr>
                        </thead>
                            ...
                            {paginatedData.length === 0 ? (
                                <tr>
                                    <td colSpan={8} className="px-4 py-8 text-center text-gray-500">
                                        No Level 2 players found. Select players in Level 1 using the bulk action.
                                    </td>
                                </tr>
                            ) : (
